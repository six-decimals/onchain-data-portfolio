...
For the 142 accepted repair keys, the fixed query uses:
...
Every other row retains the legacy fee base. This package does not apply the input-side rewrite to unreviewed keys.
...
## 10. 口径裁决 — Metric policy decisions
...
**Error →** The documentation described the input-side formula as the fixed query rule without limiting it to the 142 accepted repair keys.

**How discovered →** The final cross-file review compared the wording with Dune query 8080408, which proves that only 142 rows change and 3,105,739 rows remain unchanged.

**Correction →** I limited the formula statement to the 142 accepted keys and stated that every other row retains the legacy fee base.
