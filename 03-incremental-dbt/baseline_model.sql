-- Dune query: https://dune.com/queries/8075341/
-- File: baseline_model.sql
-- Purpose:
--   Preserve the repaired Uniswap V3 fees and protocol-revenue model
--   before converting its full-refresh pattern into an incremental model.
-- Scope:
--   Ethereum, Uniswap V3, UTC [2026-06-14, 2026-07-14).
-- Frozen expected result from Dune query 8075341:
--   trade_rows = 3,105,881
--   repaired_rows = 142
--   ordinary_repaired_rows = 140
--   material_repaired_rows = 2
--   unpriced_fee_base_rows = 12,314
--   repaired_null_rows = 0
--   fees_usd = 7,437,622.456387
--   supply_side_fees_usd = 6,020,279.185686
--   revenue_usd = 1,417,343.270700
--   accounting_residual_usd = 0.000000
-- Logical scan inventory from Dune query 03-01E at
-- 2026-07-27 07:07:04.457 UTC:
--   PoolCreated: 71,570 rows, unbounded in the baseline SQL.
--   SetFeeProtocol: 71,381 rows, unbounded in the baseline SQL.
--   dex.trades: 3,105,881 rows in the frozen 30-day window.
--   Target-pool price branch: 256 trades, all 256 minute prices matched.
